...
describe('TrainingService', () => {
    ...
    it('Stream exercise when call exerciseChangedNotifier with exercise', () => {
        scheduler.run(({ expectObservable, cold }) => {
            ...
            cold('-a').subscribe(() =>
                sut.exerciseChangedNotifier(response[0])
            );
            expectObservable(sut.exerciseChangedGetter()).toBe('-a', {
                a: response[0],
            });
        });
    });
    ...
    it('Stream exercises array when call exercisesChangedNotifier with exercises', () => {
        scheduler.run(({ expectObservable, cold }) => {
            ...
            expectObservable(sut.exercisesChangedGetter()).toBe('-a', {
                a: response,
            });
        });
    });

    it('Stream finished exercises array when call exercisesChangedNotifier with finished exercises', () => {
        scheduler.run(({ expectObservable, cold }) => {
            // sut.exerciseChangedNotifier(exercise);
            cold('-a').subscribe(() =>
                sut.finishedExercisesChangedNotifier(response)
            );
            expectObservable(sut.finishedExercisesChangedGetter()).toBe('-a', {
                a: response,
            });
        });
    });
    ...
    describe('Get available exercises && start && complete exercise', () => {
        ...
        it('Get available exercises', async () => {
            // Now verify emitted valued.
            expect((sut as any)._availableExercises).toBe(response);
            expect(uiServiceSpy.loadingStateNotifier).toHaveBeenCalledWith(
                false
            );
            expect(uiServiceSpy.showSnackBar).not.toHaveBeenCalled();
        });

        it('Start exercises successfully', async () => {
        ...
        it('Complete exercise successfully', async () => {
        ...
        it('Complete exercise passes through error', async () => {
        ...
        it('Cancel exercise successfully', async () => {
            sut.cancelExercise(3);
            expect(sut.exerciseChangedNotifier).toHaveBeenCalledWith(undefined);
        });

        it('Cancel exercise successfully with defined _runningExercise', async () => {
    ...
    it('Get available exercises passes through empty response', async () => {
        const response: any = [];
    ...
    it('Get available exercises passes through error response', async () => {
        // const status = 500;
        // const statusText = 'Internal Server Error';
        // const errorEvent = new ErrorEvent('API error');
    ...
    it('get completed or canceled exercises successfully', () => {
        const response = [
            {
                id: '15',
                name: 'test',
    ...
    it('get completed or canceled exercises passes through error response', () => {
        // const status = 500;
        // const statusText = 'Internal Server Error';
        // const errorEvent = new ErrorEvent('API error');
