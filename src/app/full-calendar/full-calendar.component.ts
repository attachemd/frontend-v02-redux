...
export class FullCalendarComponent implements OnInit, DoCheck {
    ...
    @ViewChild('fcEventContent', { static: true })
    ...
    ngOnInit(): void {
        ...
        //     this.defaultConfigurations
        // );
        const externalEventsElem = this.elRef.nativeElement
            .closest('.container')
            .querySelector('#external-events');
        let checkbox = this.elRef.nativeElement
            .closest('.container')
        ...
        this.calendar = new Calendar(calendarEl, {
            ...
            views: {
                dayGridMonth: {
                    ...
                    eventContent: (arg) =>
                        this.renderEventDayGridMonthContent(arg),
            ...
            viewDidMount: (args) => {
                ...
                this.toolbarTitle =
                    $('#full-calendar').find('.fc-toolbar-title');
            ...
            eventClassNames: (arg) => {
                if (arg.event.extendedProps.isUrgent) return ['urgent'];
                else return ['myclassname'];
            },
    ...
    devLoopThroughAllDOMElements(): void {
        ...
        for (let i = 0, max = all.length; i < max; i++) {
            ...
            console.log(all[i]);
            console.log(
                window.getComputedStyle(all[i]).getPropertyValue('opacity')
            );
        }
    }
            ...
            for (const mutation of mutationsList) {
                ...
                if (mutation.type === 'childList') {
                    ...
                    if (mutation['removedNodes'][0]) {
                        if (
                            mutation['removedNodes'][0]['nodeValue'].replace(
                                /\r|\n|\s/g,
                                ''
                            ) != ''
                        )
                        ...
                        console.log(
                            'MutationObserverWay: previous value is ',
                            mutation['removedNodes'][0]['nodeValue']
                        );
                        console.log(
                            'MutationObserverWay: previous value is ',
                    ...
                    if (mutation['addedNodes'][0]) {
                        if (
                            mutation['addedNodes'][0]['nodeValue'].replace(
                                /\r|\n|\s/g,
                                ''
                            ) != ''
                        ) {
                        ...
                        console.log(
                            'MutationObserverWay: new value is ',
                            mutation['addedNodes'][0]['nodeValue']
                        );
                        console.log(
                            'MutationObserverWay: new value is ',
    ...
    formatTitleDate(toolbarTitle: string) {
        ...
        let month: RegExpMatchArray = toolbarTitleCurrentDate
            .toLowerCase()
        ...
        if (month?.length === 2 && month[0] === month[1])
            ...
            formattedDate =
                this.getLetrDiv(month[0]) + this.getNumDiv(dayOrYear[0]);
    ...
    renderEventContent(arg: any) {
        const componentFactory =
            this.componentFactoryResolver.resolveComponentFactory(
                EventComponent
            ); // Your dynamic component will replace DynamicComponent
